...
from .utils import (distances_from_embeddings, get_children, get_embeddings,
                    ...
                    indices_of_nearest_neighbors_from_distances, split_text, semantic_chunking)
...
class TreeBuilderConfig:
    def __init__(
        ...
        if summarization_model is None:
            from .SummarizationModels import OllamaSummarizationModel
            summarization_model = OllamaSummarizationModel()
        ...
        if chunking_strategy not in ["token", "semantic"]:
            raise ValueError("chunking_strategy must be either 'token' or 'semantic'")
        self.chunking_strategy = chunking_strategy
        
        if not isinstance(chunking_threshold, (int, float)) or not (0 <= chunking_threshold <= 1):
            raise ValueError("chunking_threshold must be a number between 0 and 1")
        self.chunking_threshold = chunking_threshold
        
        if not isinstance(use_ice, bool):
            raise ValueError("use_ice must be a boolean")
...
class TreeBuilder:
    ...
    def __init__(self, config) -> None:
        ...
        self.chunking_threshold = config.chunking_threshold
        self.use_ice = config.use_ice
    ...
    def build_from_text(self, text: str, use_multithreading: bool = True) -> Tree:
        ...
        if self.chunking_strategy == "semantic":
            chunks = semantic_chunking(
                text,
                self.embedding_models[self.cluster_embedding_model],
                self.tokenizer,
                self.max_tokens,
                threshold=self.chunking_threshold
            )
        else:
            chunks = split_text(text, self.tokenizer, self.max_tokens)
            
        if self.use_ice:
            logging.info("Applying Intrinsic Contextual Enrichment (ICE) to chunks...")
            def enrich_chunk(chunk):
                context_tag = self.summarization_model.extract_context(chunk)
                if context_tag:
                    return f"Context: {context_tag} | Text: {chunk}"
                return chunk
            
            # Extract contexts in parallel since they usually invoke an LLM API
            with ThreadPoolExecutor() as executor:
                chunks = list(executor.map(enrich_chunk, chunks))
