...
def inject_positional_dimension(reduced_embeddings, lambda_weight=0.25):
    """
    reduced_embeddings: The output from UMAP (Shape: [N, D])
    lambda_weight: How much the 'Page Number' should matter
    """
    num_nodes = reduced_embeddings.shape[0]
    positional_indices = np.linspace(0, 1, num_nodes).reshape(-1, 1)
    weighted_position = positional_indices * lambda_weight
    enhanced_embeddings = np.hstack((reduced_embeddings, weighted_position))
...
def global_cluster_embeddings(
    ...
    # Prevent UMAP issues when chunks are very small (like our 4-chunk proof)
    if n_neighbors >= len(embeddings):
        n_neighbors = max(1, len(embeddings) - 1)
        
    reduced_embeddings = umap.UMAP(
        n_neighbors=n_neighbors, n_components=dim, metric=metric
    ).fit_transform(embeddings)
    
    # Inject positional dimension
    enhanced_embeddings = inject_positional_dimension(reduced_embeddings, lambda_weight=0.25)
    return enhanced_embeddings
...
def local_cluster_embeddings(
    ...
    if num_neighbors >= len(embeddings):
        num_neighbors = max(1, len(embeddings) - 1)
