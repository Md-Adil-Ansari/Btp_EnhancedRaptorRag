...
def semantic_chunking(
    ...
    """
    Splits the input text into semantic chunks based on the embedding model, tokenizer, and maximum allowed tokens.
    
    Args:
        text (str): The text to be split.
        embedding_model (BaseEmbeddingModel): The model used to calculate sentence embeddings.
        tokenizer (CustomTokenizer): The tokenizer to be used for splitting the text.
        max_tokens (int): The maximum allowed tokens.
        overlap (int, optional): The number of overlapping sentences between chunks. Defaults to 0.
        threshold (float, optional): The cosine distance threshold to split sentences. Defaults to 0.5.
    
    Returns:
        List[str]: A list of text chunks.
    """
    import re
    # Replace newlines with spaces to avoid splitting on them
    text = text.replace('\n', ' ')
    # Split the text into sentences avoiding common abbreviations (Dr., Mr., etc.)
    sentences = re.split(r'(?<!\b[A-Za-z]\.)(?<!\b[A-Za-z][a-z]\.)(?<=[.!?])\s+', text)
    ...
    embeddings = [embedding_model.create_embedding(sentence) for sentence in sentences]
    
    # Calculate cosine distances between adjacent sentences
    distances = []
    for i in range(len(embeddings) - 1):
        dist = spatial.distance.cosine(embeddings[i], embeddings[i+1])
        distances.append(dist)
        
    chunks = []
    current_chunk = [sentences[0]]
    current_length = n_tokens[0]
    
    for i in range(len(distances)):
        dist = distances[i]
        next_sentence = sentences[i+1]
        token_count = n_tokens[i+1]
        
        # If the sentence is too long by itself
        if token_count > max_tokens:
            # We must break it (for simplicity, we let the token length check split the chunk before it)
            pass
            
        # Check if we should split based on semantic distance or max tokens
        if dist > threshold or current_length + token_count > max_tokens:
            if current_chunk:
                chunks.append(" ".join(current_chunk))
